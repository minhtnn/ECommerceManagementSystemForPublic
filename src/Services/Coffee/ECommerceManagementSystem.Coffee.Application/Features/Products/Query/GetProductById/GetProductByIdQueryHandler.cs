...
public class GetProductByIdQueryHandler : IRequestHandler<GetProductByIdQuery, ApiResponse>
{
    ...
    public GetProductByIdQueryHandler(IUnitOfWork<ECommerceManagementSystemCoffeeContext> unitOfWork, ILogger logger,
        IMediaService mediaService, ICacheInvalidationService cacheService, IClaimService claimService) : base()
    {
    ...
    public async ValueTask<ApiResponse> Handle(GetProductByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var cachedProducts = await _cacheService.GetDetailFromCacheAsync<GetProductByIdResponse>(
                $"{CacheConfig.EntityByIdCachePrefix(nameof(Domain.Entities.Products), request.ProductId.ToString())}:{role}:{brandId.ToString()}"
            );
